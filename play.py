...
Run this file to play against any bot in the bots/ directory.

Usage:
    python play.py [bot_name] [--time-limit SECONDS] [--position FILE]
...
from evaluate import load_bot, list_bots
...
def _get_ai_stats(bot):
    """Extract optional stats from the underlying bot object, if available."""
    obj = bot._bot
    if obj is None:
        return None
    depth = bot.last_depth
    nodes = getattr(obj, '_nodes', None)
    score = getattr(obj, 'last_score', None)
    if nodes is not None and score is not None:
        return (depth, nodes, score)
    return None


def _get_pv(bot):
    """Extract principal variation from the underlying bot, if available."""
    obj = bot._bot
    if obj is None:
        return []
    score = getattr(obj, 'last_score', None)
    if score is not None and abs(score) >= 99_999_000 and hasattr(obj, 'extract_pv'):
        pv = obj.extract_pv()
        pv_display = []
        for step in pv:
...
def main():
    parser = argparse.ArgumentParser(
        description="Play Hex Tic-Tac-Toe against any bot.",
        epilog=f"Available bots: {', '.join(list_bots())}")
    parser.add_argument("bot", nargs="?", default="SealBot",
                        help="Bot name from bots/ directory (default: SealBot)")
    ...
    pygame.display.set_caption(f"Hex Tic-Tac-Toe \u2014 vs {ai.name}")
        ...
        if (mode == MODE_PLAY
            ...
            for q, r in moves:
                if not game.game_over:
                    ...
                    move_history.append((q, r))
                    history_pos = len(move_history)
            ai_stats = _get_ai_stats(ai)
            pv_display = _get_pv(ai)
